...
- Dropped SK_ID_CURR, 19 FLAG_DOCUMENT columns, and ~45 building/property measurement columns (48–70% missing, no credit signal)
...
- EXT_SOURCE_2 and EXT_SOURCE_3 identified as strongest numerical predictors (−0.16 correlation with target each); EXT_SOURCE_1 missing for 56.38% of applicants — the single largest data quality gap in the dataset
...
- StandardScaler fit on training data only — no leakage into validation or test
...
- Threshold tuning across 0.30–0.50: threshold 0.40 selected as the optimal operating point — peaks on F1, balancing the asymmetric cost of missed defaults (unrecoverable) versus false rejections (recoverable opportunity cost)
- EXT_SOURCE_1 missingness quantified: 173,271 applicants scored with the 3rd strongest predictor replaced by a constant — fixing the data pipeline is the single highest-ROI improvement available with zero architectural change
- Three-tier risk segmentation deployed; SHAP TreeExplainer used for global and local interpretability
...
| Test Recall | 39.92% | At threshold 0.40 — F1-optimal operating point |
...
| Recall at threshold 0.30 | 70.92% | 3,521 defaults caught — but 18,597 false alarms |
| Recall at threshold 0.40 | 55.13% | 2,737 defaults caught — 11,284 false alarms (F1 peak) |
...
| EXT_SOURCE_1 Missing | 56.38% | 173,271 applicants scored with zero discriminative signal |
...
- **Fix EXT_SOURCE data pipeline** — 173,271 applicants scored with EXT_SOURCE_1 replaced by a constant. Fixing the data feed directly improves discrimination with zero architectural change.
...
- 📊 **Executive Presentation:** 14-slide business storytelling deck — findings, threshold decision, risk tiers, SHAP interpretability, and recommendations
